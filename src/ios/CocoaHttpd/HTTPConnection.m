...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    @try
    {
        path = [NSString stringWithFormat:@"http://dummy%@",path]; //just to use NSURL impl
        NSURL *_path = [NSURL URLWithString:path];

        NSString *endPoint = [_path path];
        endPoint = [endPoint stringByReplacingOccurrencesOfString:@"/" withString:@""];
        NSString *param = [_path query];
        
        if(endPoint == nil || endPoint.length == 0) //ignore if there is no endpoint
            return nil;
        
        NSMutableDictionary *pluginResult = [NSMutableDictionary new];
        
        //add end point
        [pluginResult setObject:endPoint forKey:@"endPoint"];
        
        NSArray *paramsUgly = [param componentsSeparatedByString:@"&"];
        NSMutableDictionary *paramFormatted = [NSMutableDictionary new];
        for (NSString *param in paramsUgly)
        {
            NSString *key = [[param componentsSeparatedByString:@"="] firstObject];
            NSString *value = [[param componentsSeparatedByString:@"="] lastObject];
            
            [paramFormatted setObject:value forKey:key];
        }
        
        //add param
        [pluginResult setObject:paramFormatted forKey:@"param"];
        
        dispatch_async(dispatch_get_main_queue(), ^{
            [config.server sendIncomingConnectionInfo:pluginResult];
        });
    }
    @catch (NSException *exception)
    {
        NSLog(@"Exception occured while trying to parse incoming connection, uri");
    }
